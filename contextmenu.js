            ...
            'afteractivate': function(p) {
                ...
                if (p.ref.menu_name == 'contactlist') {
                    ...
                    if (!rcmail.env.group || rcmail.env.readonly)
                        p.ref.container.find('a.cmd_group-remove-selected').removeClass(rcmail.contextmenu.settings.classes.button_active).addClass(rcmail.contextmenu.settings.classes.button_disabled);
                    ...
                    if (groupcount > 0)
                        p.ref.container.find('a.assigngroup').removeClass(rcmail.contextmenu.settings.classes.button_disabled).addClass(rcmail.contextmenu.settings.classes.button_active);
                    else
                        p.ref.container.find('a.assigngroup').removeClass(rcmail.contextmenu.settings.classes.button_active).addClass(rcmail.contextmenu.settings.classes.button_disabled);
                }
...
function rcm_addressbook_selector(event, command, callback) {
    ...
    if (!container) {
        ...
        container = $('<div>').attr('id', 'addressbook-selector').addClass(rcmail.contextmenu.settings.classes.container + ' ' + rcmail.contextmenu.settings.classes.popupmenu);
...
function rcm_group_selector(event, command, callback) {
    ...
    if (!container) {
        ...
        container = $('<div>').attr('id', 'addressgroup-selector').addClass(rcmail.contextmenu.settings.classes.container + ' ' + rcmail.contextmenu.settings.classes.popupmenu);
...
function rcm_addressbook_selector_item(obj, abook_id, parent) {
    if (abook_id && abook_id === obj.source || !abook_id) {
        ...
        if (obj.type == 'group') {
            a.addClass(rcmail.contextmenu.settings.classes.button_active + ' contactgroup')
            a.data('source', obj.source);
            a.data('id', obj.id);
            a.css('padding-left', '16px');
        }
        else {
            a.addClass(rcmail.contextmenu.settings.classes.button_active + ' addressbook').data('id', obj.id);
        }
